...
- Sharing the domain is safe while the Worker only sends Handoff notifications to Nathan, since no mail goes to addresses that could bounce or complain. If the Worker ever sends to Visitor addresses (relaying Nathan's answers so they can become Proposals), Handoff mail moves to its own sending domain first, to keep the newsletter's reputation separate.
