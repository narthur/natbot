...
// About $2/day at ~5k input and ~300 output tokens per answer.
...
export class ChatAgent extends AIChatAgent<Env> {
  ...
  async onChatMessage(_onFinish: unknown, options?: OnChatMessageOptions) {
    ...
    const result = streamText({
      ...
      onFinish: ({ text }) => {
        this.sql`INSERT INTO turns (question, answer) VALUES (${question}, ${text})`;
      },
    });
    // The default hides error details from the client; this gives the visitor something to act on.
    return result.toUIMessageStreamResponse({ onError: () => "Sorry, something went wrong. Please try again." });
  }
}
