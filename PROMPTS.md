...
Every prompt Nathan typed is included verbatim, including pasted text. Not included: prompts generated by automation (the security-guidance plugin's review of each commit) or written by Claude for its own subagents. [docs/process.md](docs/process.md) describes that and the rest of the scaffolding around these prompts. Times are US Central; the first three entries were backfilled when the hook was added, so they have no seconds.
